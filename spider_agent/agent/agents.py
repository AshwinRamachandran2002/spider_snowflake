...
from spider_agent.agent.action import Action, Terminate, SNOWFLAKE_REGISTER_RELEVANCE_OF_TABLES, SNOWFLAKE_EXEC_SQL, SNOWFLAKE_Yes_NO, SNOWFLAKE_MODIFY_CTE, SNOWFLAKE_REGISTER_CTE, SNOWFLAKE_READ_TABLE_SCHEMA_FROM_JSON, SNOWFLAKE_READ_SCHEMA_FROM_DDL, SNOWFLAKE_JUSTIFY_JSON_FILE_RELEVANCE, SNOWFLAKE_JUSTIFY_RELEVANT_JSON_FILE_RELEVANCE, SNOWFLAKE_JUSTIFY_DDL_RELEVANCE,SNOWFLAKE_REGISTER_RELEVANCE_OF_RELEVANT_COLUMNS_FOR_CTE, SNOWFLAKE_REGISTER_RELEVANCE_OF_ALL_COLUMNS_FOR_TABLE, PREDICTED_MINIMAL_SET_OF_COLUMN_NAMES_AND_EXAMPLE_ROWS, SNOWFLAKE_CHECK_IF_CONDITIONAL_CLAUSES_WORK, SNOWFLAKE_FIND_DISTINCT_VALUES_IN_THE_COLUMN
...
class PromptAgent:
    ...
    def set_env_and_task(self, env: Spider_Agent_Env):
        ...
        self._AVAILABLE_ACTION_CLASSES = [Terminate, SNOWFLAKE_REGISTER_RELEVANCE_OF_TABLES, SNOWFLAKE_EXEC_SQL, SNOWFLAKE_Yes_NO, SNOWFLAKE_MODIFY_CTE, SNOWFLAKE_REGISTER_CTE, SNOWFLAKE_CHECK_IF_CONDITIONAL_CLAUSES_WORK, SNOWFLAKE_READ_TABLE_SCHEMA_FROM_JSON, SNOWFLAKE_READ_SCHEMA_FROM_DDL, PREDICTED_MINIMAL_SET_OF_COLUMN_NAMES_AND_EXAMPLE_ROWS,SNOWFLAKE_REGISTER_RELEVANCE_OF_RELEVANT_COLUMNS_FOR_CTE, SNOWFLAKE_REGISTER_RELEVANCE_OF_ALL_COLUMNS_FOR_TABLE, SNOWFLAKE_FIND_DISTINCT_VALUES_IN_THE_COLUMN, SNOWFLAKE_JUSTIFY_DDL_RELEVANCE, SNOWFLAKE_JUSTIFY_JSON_FILE_RELEVANCE, SNOWFLAKE_JUSTIFY_RELEVANT_JSON_FILE_RELEVANCE]
    ...
    def run(self):
        ...
        while not done and step_idx < self.max_steps:
            ...
            import pickle
            if loaded_ok == 0 and os.path.exists(f"data_dump2/{self.instruction_id}.pkl"):
                with open(f"data_dump2/{self.instruction_id}.pkl", "rb") as f:
                    ...
                    obs = self.env.relevance_table_before_exec()
                    # self.model = "o1"
            ...
            if action is None:
                ...
                if refresh == "Change to o1":
                    self.model = "gpt-4o"
                elif refresh == "Change to gpt-4o":
                    self.model = "gpt-4o"
                elif refresh == "Go back to JSON justification":
                    ...
                    if os.path.exists(f"data_dump2/{self.instruction_id}.pkl"):
                        os.remove(f"data_dump2/{self.instruction_id}.pkl")
                    if not os.path.exists("data_dump2"):
                        os.makedirs("data_dump2")
                    with open(f"data_dump2/{self.instruction_id}.pkl", "wb") as f:
            ...
            if done:
                if os.path.exists(f"sql_dump2/{self.instruction_id}.pkl"):
                    os.remove(f"sql_dump2/{self.instruction_id}.pkl")
                if not os.path.exists("sql_dump2"):
                    os.makedirs("sql_dump2")
                with open(f"sql_dump2/{self.instruction_id}.pkl", "wb") as f:
                    pickle.dump({"indi_sqls": self.env.indi_sqls, "tables": self.env.tables, "last_major_sql": self.env.last_major_sql,
                                    "final_exec": self.env.final_exec, "indi_sqls_exec": self.env.indi_sqls_exec}, f)
