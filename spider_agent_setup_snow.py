...
JSONL_PATH = '../../../Spider2-new/spider2-snow/spider2-snow.jsonl'
DATABASE_PATH = '../../../Spider2-new/spider2-snow/resource/databases/'
DOCUMENT_PATH = '../../../Spider2-new/spider2-snow/resource/documents'
...
def setup_special_functions(
    ...
    for example in examples:
        ...
        folder_path = f'examples/{instance_id}'
        for file in files:
            file_path = 'tree_function.py'
            target_function_path = os.path.join(folder_path, file_path)

            if os.path.exists(target_function_path):
                os.remove(target_function_path)

            shutil.copy(file_path, target_function_path)
        print(f"Copied functions for instance {instance_id}.")
...
def refine_jsonl(refine_jsonl):
    with open(JSONL_PATH, "r") as f:
        examples = [json.loads(line) for line in f]

    refined_examples = json.load(open(refine_jsonl, "r"))
    refined_ids = [example['id'] for example in refined_examples]

    examples_new = []
    for example in examples:
        if example['instance_id'] in refined_ids:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--refine_jsonl", help="Refine JSONL")

    args = parser.parse_args()

    if args.refine_jsonl:
        refine_jsonl(args.refine_jsonl)

    add_snowflake_agent_setting()

    setup_snowflake()
    setup_special_functions()
